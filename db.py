...
def get_bot_for_telegram(bot_id: str) -> Optional[dict]:
    ...
    pick up the bot config in one round trip."""
    if not bot_id:
        return None
    try:
        result = (
        ...
        print(f"[db] get_bot_for_telegram failed: {e}")
    return None
...
def update_bot_telegram(
    ...
    *,
    ...
    """Persist Telegram credentials on a bot row (or clear them when token is
    None — used during disconnect)."""
    if not bot_id:
        return
    try:
        client().table("bots").update({
            ...
            "telegram_webhook_set_at": (
                None if token is None else _utcnow_iso()
            ),
        }).eq("id", bot_id).execute()
    except Exception as e:
        print(f"[db] update_bot_telegram failed: {e}")
